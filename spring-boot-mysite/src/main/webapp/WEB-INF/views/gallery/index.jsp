		...
		<div id="content">
			<div id="gallery">
				...
				<ul>
					...
					<li>
						<a	href="${pageContext.request.contextPath }${vo.imageURL }" title="${vo.comment}"
						data-lightbox="gallery"
							class="image"
							style="background-image:url('${pageContext.request.contextPath }${vo.imageURL }')">&nbsp;</a>
						<c:if test="${not empty authuser && authuser.role=='ADMIN' }">
							<a href="gallery/delete?no=${vo.no}&url=${vo.imageURL }" class="del-button" title="삭제">삭제</a>
						</c:if>
					</li>
					</c:forEach>
				</ul>
			...
			<c:if test="${not empty authuser && authuser.role=='ADMIN' }">
				<div id="dialog-upload-form" title="이미지 업로드" style="display: none">
					...
					<form action="${pageContext.request.contextPath }/gallery/upload" method="post" enctype="multipart/form-data">
						...
						<label>이미지</label> <input type="file" id="input-file" name="file" multiple accept='image/*'> 
